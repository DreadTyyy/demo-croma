...
export default function ButtonPrimary({bgColor,shadow,hoverShadow,handleClick, children}
  ...
  return (
    <button onClick={handleClick}
      className={`${_bgColor} ${_shadow} ${_hoverShadow} px-4 lg:px-6 2xl:px-8 py-3 lg:py-4 2xl:py-5  rounded-[10px] cursor-pointer text-sm lg:text-base transition-all duration-300`}>
        {children}
    </button>
  );
}
